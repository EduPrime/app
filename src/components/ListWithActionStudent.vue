...
import { arrowBack, arrowForward, eye, pencilSharp, schoolSharp, trashSharp } from 'ionicons/icons'
  ...
  <IonCard v-if="true">
    <IonCardHeader color="secondary">
      <div style="display: flex; align-items: center; height: 15px;">
        <IonIcon :icon="schoolSharp" style="margin-right: 10px; font-size: 16pt;" />
        <IonCardTitle style="font-size: medium;">
          Lista de estudantes ({{ props.dataList.length }})
        </IonCardTitle>
      </div>
    </IonCardHeader>
      ...
      <IonCardContent class="ion-no-padding">
        <IonList>
          <div v-if="innerWidth < 992">
            ...
            >
              <IonItem>
                <IonLabel color="primary" class="ion-no-margin student-name" style="margin-top: 5px; margin-bottom: 5px;">
                  <h2>{{ item.name || 'Estudante sem nome' }}</h2>
                  <p v-if="item.email">
                    {{ item.email }}
                  </p>
                  <p v-if="item.phone">
